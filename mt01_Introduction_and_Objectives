...
This Intermodular Project arises from the need to update our educational center's network infrastructure due to the obsolescence of the current proxy system and the demand for a more secure and efficient architecture.

Taking advantage of the replacement of the edge device (proxy), the logical topology will be redesigned to transition from a flat network to a segmented and hierarchical structure. The centerpiece of this upgrade is the implementation of a Demilitarized Zone (DMZ), designed to host publicly accessible services and isolate external traffic from the educational and administrative internal network (LAN).

The project simulates a real production environment for a Vocational Training institute, integrating advanced routing, services such as DNS, and automated deployment systems (FOG Project).

## 1.2. Project Objectives

### General Objective
Design, implement, and secure a scalable network infrastructure that replaces the current proxy system, integrating perimeter security through a DMZ and optimizing service management for the training cycles (FPB, SMR, and ASIR).

### Specific Objectives

#### 1. Restructuring of the Network Topology
...
#### 2. Implementation of Services in the DMZ
* **DNS:** Configuration of name resolution and web content filtering, optimizing bandwidth through caching.
* **Web:** Creation of a webpage containing the project's contents.
* **Database (BBDD):** Creation of a user and a database linked to the web server.
* **LDAP:** Authentication server for Proxmox and WordPress.
* **OPNsense:** Routing and network management service.

#### 3. Optimization of the Internal Network (Management Services)
...
* **Implementation:** Installation and configuration of services (DNS, Web, Database, FOG, LDAP) on server operating systems (Linux/Windows Server).
...
* Evaluation of existing hardware resources (CPU, RAM, Storage) on servers intended for virtualization (Containers, Docker, Kubernetes, Proxmox/ESXi) to ensure they support the load of the new services (FOG, Web, etc.).
